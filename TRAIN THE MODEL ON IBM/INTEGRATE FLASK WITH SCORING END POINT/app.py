...
import pickle
model = pickle.load(open(r'C:\Users\HP\Downloads\MultiLinear Regression\Flask app\model.pkl','rb'))

@app.route('/')#binds to an url
def helloworld():
    return render_template("index.html")

@app.route('/login')#binds to an url
def login():
...
def login():
    ...
    return render_template("index.html", prediction = "the predicted profit is of your startup " + str(output[0][0]) )
